...
Sediakan 5 Soal PG Remedial lengkap dengan kunci, Pengayaan, Glosarium, dan Daftar Pustaka.
Balas HANYA JSON:
...
def prompt_promes(form, aggregated_context):
    ...
    return f"""Buat Program Semester Lengkap Mapel {form['mapel']} {form['kelas']}. Semester: {sem}.\nEkstrak rincian Sub-Bab dari master konteks dan jadikan baris dengan 'jenis': 'sub'.\n{aggregated_context}\nBalas HANYA JSON:\n{{"tables": [{{"judul_semester": "SEMESTER 1", "bulan": ["Juli", "Agustus", "September"], "rows": [{{"jenis": "bab", "materi": "Bab 1", "jp": "30", "distribusi": []}}, {{"jenis": "sub", "materi": "A. Nama", "jp": "10", "distribusi": [{{"bulan": "Juli", "minggu": 1, "jp": "4"}}]}}]}}]}}"""
...
def build_modul_ajar(form: dict, bab_name: str, jumlah_pertemuan: int, full_data: dict) -> bytes:
    ...
    for p in pertemuan_list:
        ...
        style_cell(r_pem[2], pem_data.get("waktu", "10'"), center=True); style_cell(r_pem[3], "MEANINGFUL", center=True)
        doc.add_paragraph()
        ...
        inti_phases = [("MEMAHAMI", p.get("inti_memahami", {}), "MINDFUL"), ("MENGAPLIKASIKAN", p.get("inti_mengaplikasikan", {}), "JOYFUL"), ("MEREFLEKSIKAN", p.get("inti_merefleksikan", {}), "MEANINGFUL")]
        ...
        style_cell(r_pen[2], pen_data.get("waktu", "10'"), center=True); style_cell(r_pen[3], "MINDFUL", center=True)
        doc.add_paragraph()
    ...
    style_cell(r_n2[0], "Asesmen Formatif (As & For Learning)", bold=True); set_cell_background(r_n2[0], "D6EAF8")
...
def build_lkpd_per_bab(form, bab_name, d5_lkpd_data):
    ...
    for item in safe_list(d5_lkpd_data.get("lkpd", [])):
        ...
        doc.add_heading(str(item.get("topik_judul", f"Topik Pertemuan {item.get('pertemuan', '1')}")), level=2)
        ...
        for act in safe_list(item.get("aktivitas", [])):
            ...
            doc.add_heading(act.get("judul_aktivitas", "Aktivitas"), level=3)
